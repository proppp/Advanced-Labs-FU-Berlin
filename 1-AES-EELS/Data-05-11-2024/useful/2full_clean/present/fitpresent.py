...
def plot_txt_files_with_multiple_fits(folder_path, ranges, energy_correction=15.74):
    # Apply energy correction to each range
    ...
    for file_name in txt_files:
        ...
        if data:
            ...
            plt.title("Energy Spectrum for Cleaned Sample (Without Gaussian Fits)", fontsize=16)
            ...
            # Save plot as PNG file for Google Docs compatibility
            output_file_path = os.path.join(folder_path, f"{file_name}_no_fit_plot.png")
            ...
            # Plot the original data with Gaussian derivative fits
            plt.figure(figsize=(10, 6))
            plt.plot(df['X'], df['Y'], linestyle='-', color='b', label='Data')
            plt.title("Energy Spectrum for Cleaned Sample with Gaussian Derivative Fits", fontsize=16)
            plt.xlabel("Energy (eV)", fontsize=14)
            plt.ylabel("dN/dE (a.u.)", fontsize=14)
            #plt.grid()
            plt.xticks(fontsize=12)
            ...
            # Save plot as PNG file for Google Docs compatibility
            output_file_path = os.path.join(folder_path, f"{file_name}_fit_plot.png")
            plt.savefig(output_file_path, format='png', dpi=300)
            plt.show()
...
ranges = [(10, 105), (0, 0), (0, 0), (1367, 1400)]  # Specify the four ranges here
