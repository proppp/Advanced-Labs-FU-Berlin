...
m_e_c2 = 511  # Electron rest mass energy in keV (for Compton edge calculation)
...
def calculate_compton_edge(peak_energy):
    """Calculates the Compton edge based on the peak energy."""
    return peak_energy * (1 - 1 / (1 + peak_energy / m_e_c2))

def plot_sodium_spectra_with_peaks_and_compton(folder_path, files, peak_ranges):
    """Plots the spectra for sodium-22 detectors, identifies main peaks, and marks the Compton edge."""
    ...
    for file_name, ranges in zip(files, peak_ranges):
        try:
            ...
            for peak_range in ranges:
                ...
                # Annotate the peak and calculate Compton edge
                plt.annotate(f"{params[1]:.2f} keV", (params[1], max(fit_counts)),
                             textcoords="offset points", xytext=(0, 10), ha='center')

                # Calculate and plot the Compton edge
                compton_edge = calculate_compton_edge(params[1])
                plt.axvline(compton_edge, color='red', linestyle=':', label=f"Compton edge: {compton_edge:.2f} keV")
                plt.annotate(f"Compton edge\n{compton_edge:.2f} keV",
                             (compton_edge, max(fit_counts) * 0.8),
    ...
    # Customize plot
    plt.title("Sodium-22 Spectra, Identified Peaks, and Compton Edges")
    ...
    # Save plot as a PDF
    output_filename = "sodium_spectra_peaks_compton_edges.pdf"
...
peak_ranges = [
    [(500, 550), (1200, 1300)],  # Example ranges for Detector 1
    [(500, 550), (1200, 1300)]   # Example ranges for Detector 2
]

# Plot the spectra, identify peaks, and mark the Compton edges
plot_sodium_spectra_with_peaks_and_compton(folder_path, files, peak_ranges)
