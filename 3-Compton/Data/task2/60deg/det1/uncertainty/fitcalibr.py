...
def parse_spectrum_file(filepath):
    ...
    Returns channels, scaled counts, total counts, and measurement time.
    """
    channels = []
    counts = []
    ...
    # Compute total counts per second
    total_counts_per_second = total_counts

    # Scale counts by measurement time
    scaled_counts = [count / measurement_time for count in counts]

    return channels, scaled_counts, total_counts_per_second
...
def plot_spectra_in_folder(folder_path):
    ...
    for file in files:
        ...
        try:
            channels, scaled_counts, total_counts_per_second = parse_spectrum_file(file_path)
            energies = calibrate_channels_to_energy(channels)
            fwhm = find_fwhm(energies, scaled_counts)

            # Print total counts per second in the console
            print(f"File: {file}, Total Counts per Second: {total_counts_per_second:.2f}")

            # Format legend entry with FWHM
