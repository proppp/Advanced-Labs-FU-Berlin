...
For this, we first optimized the $\pi$ pulse by maximizing the echo and minimizing the FID of the second pulse. Then, we varied $\tau$ from around $0.0001\text{s}$ to $0.0130 \text{s}$, and plotted the resulting spectra in a single plot, ensuring proper alignment in time. Since different scales of the oscilloscope were used, alignment in time was crucial, and it was performed by manually shifting the $\frac{\pi}{2}$ pulse so that it peaked at time $t = 0$.
