...
peak_prominence = 0.016  # You can adjust this value for marking peaks
    ...
    if filename.endswith(".dat"):
        ...
        if peaks.size > 0:
            ...
            valid_peaks = [
                ...
                if not ((y_aligned[i] < 0.1 and x_aligned[i] < 0.008) or (y_aligned[i] < 0.4 and x_aligned[i] < 0.00025))
            ]
...
plt.title(f"Spin-Echo Sequences for CuSO₄, 0.2M (Hahn Echo Method)")
plt.legend()
plt.grid(True)

# Save the plot as a PDF
output_pdf_path = "Spin_echo_0.2M.pdf"
