...
data = np.loadtxt("MG-0.0001-N80.dat")  # Update with the actual file path
...
mask = x >= 0.02  # Mask the data to include the full range initially
x_filtered = x[mask]
y_filtered = y[mask]
...
def fit_and_save_peaks():
    if len(selected_peaks) > 0:
        ...
        plt.savefig("MG_0.1.pdf", format="pdf")
        print("Plot saved as 'MG_0.1.pdf'.")
